...
整段导出包括页面已经加载的提问与回答，以及保存的高亮和批注，不受复选框选择影响。长对话请先向上滚动加载。常见公式会使用随扩展打包的字体重新排版，支持分数、根号、上下标、积分、矩阵和分段式。没有 TeX 源码时尝试保留 MathML；无法识别的公式保留可读取的内容并在预览提示。无法加载的图片也会提示。
