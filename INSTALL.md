...
下载 **ChatGPT-Marker-Safari-v0.4.3-unsigned.dmg**。这是免费测试包，没有 Apple 开发者证书签名和公证；仅使用本地临时签名（ad-hoc），让 Apple 芯片能够运行程序。
...
整段导出包括页面已经加载的提问与回答，以及保存的高亮和批注，不受复选框选择影响。长对话请先向上滚动加载。常见公式会使用随扩展打包的字体重新排版，支持分数、根号、上下标、积分、矩阵和分段式。没有 TeX 源码时尝试保留 MathML；无法识别的公式保留源码并在预览提示。无法加载的图片也会提示。
...
**ChatGPT-Marker-Safari-v0.4.3.zip** 仍可用作临时扩展测试：解压后，在 Safari 设置 → 开发者 → 添加临时扩展中选择直接包含 `manifest.json` 的 ChatGPT-Marker 文件夹。这种安装方式在退出 Safari 或 24 小时后会被移除。日常测试优先使用上面的 DMG。
